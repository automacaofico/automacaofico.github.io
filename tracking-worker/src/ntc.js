...
function normalizeNtcState(value) {
...
async function readState(request, env) {
  const row = await env.DB.prepare('SELECT data_json,revision,updated_at FROM ntc_state WHERE singleton_id=1').first();
...
async function saveState(request, env) {
  ...
  const normalized = normalizeNtcState(body.state);
  ...
  const current = await env.DB.prepare('SELECT data_json,revision,updated_at FROM ntc_state WHERE singleton_id=1').first();
  if (!current) {
    ...
    await env.DB.prepare('INSERT INTO ntc_state (singleton_id,data_json,revision,updated_at) VALUES (1,?,1,CURRENT_TIMESTAMP)')
      .bind(normalized.serialized).run();
    const created = await env.DB.prepare('SELECT revision,updated_at FROM ntc_state WHERE singleton_id=1').first();
  ...
  const result = await env.DB.prepare('UPDATE ntc_state SET data_json=?,revision=revision+1,updated_at=CURRENT_TIMESTAMP WHERE singleton_id=1 AND revision=?')
    .bind(normalized.serialized, expectedRevision).run();
  if (Number(result.meta?.changes || 0) !== 1) {
    const latest = await env.DB.prepare('SELECT revision,updated_at FROM ntc_state WHERE singleton_id=1').first();
  ...
  const saved = await env.DB.prepare('SELECT revision,updated_at FROM ntc_state WHERE singleton_id=1').first();
  return reply(request, { ok: true, revision: Number(saved.revision), updatedAt: saved.updated_at });
}

export async function routeNtc(request, env) {
  const path = new URL(request.url).pathname;
  if (!path.startsWith('/api/v1/ntc/')) return null;
  if (request.method === 'OPTIONS') return new Response(null, { status: 204, headers: cors(request) });
  if (request.method === 'GET' && path === '/api/v1/ntc/state') return readState(request, env);
  if (request.method === 'POST' && path === '/api/v1/ntc/state') return saveState(request, env);
