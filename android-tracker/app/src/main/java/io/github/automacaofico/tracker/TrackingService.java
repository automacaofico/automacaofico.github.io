...
public class TrackingService extends Service {
    ...
    private void capture(Location l){
        try{
            JSONObject p=new JSONObject(); p.put("equipmentId",BuildConfig.EQUIPMENT_ID); p.put("capturedAt", Instant.ofEpochMilli(l.getTime()).toString()); p.put("latitude",l.getLatitude()); p.put("longitude",l.getLongitude()); p.put("accuracyM",l.getAccuracy()); p.put("sequenceNo",++sequence); p.put("batteryPct",battery());
    ...
    private void upload(){ if(!uploading.compareAndSet(false,true))return; network.execute(()->{ try{ while(true){ List<QueueDb.Item> items=db.first(100); if(items.isEmpty())break; JSONArray array=new JSONArray(); for(QueueDb.Item item:items)array.put(new JSONObject(item.payload)); JSONObject body=new JSONObject().put("positions",array); HttpURLConnection c=(HttpURLConnection)new URL(BuildConfig.API_URL+"/api/v1/positions").openConnection(); c.setRequestMethod("POST"); c.setConnectTimeout(12000); c.setReadTimeout(12000); c.setRequestProperty("Authorization","Bearer "+BuildConfig.DEVICE_TOKEN); c.setRequestProperty("Content-Type","application/json"); c.setDoOutput(true); try(OutputStream out=c.getOutputStream()){out.write(body.toString().getBytes(StandardCharsets.UTF_8));} int code=c.getResponseCode(); c.disconnect(); if(code<200||code>=300)throw new IllegalStateException("HTTP "+code); db.deleteThrough(items.get(items.size()-1).id); prefs().edit().putString("lastSend","enviado agora").putInt("queue",db.count()).apply(); } }catch(Exception e){ prefs().edit().putString("lastSend","offline · aguardando conexão").putInt("queue",db.count()).apply(); }finally{uploading.set(false);} }); }
    ...
    private Notification notification(String text){ return new NotificationCompat.Builder(this,CHANNEL).setSmallIcon(R.drawable.ic_launcher).setContentTitle("NTC001 · rastreamento ativo").setContentText(text).setOngoing(true).setCategory(NotificationCompat.CATEGORY_SERVICE).build(); }
