...
public class TrackingService extends Service {
    ...
    private void capture(Location l){
        try{
            String equipment=prefs().getString("equipmentId",""); if(equipment.isEmpty()) { stopSelf(); return; }
            JSONObject p=new JSONObject(); p.put("equipmentId",equipment); p.put("capturedAt", Instant.ofEpochMilli(l.getTime()).toString()); p.put("latitude",l.getLatitude()); p.put("longitude",l.getLongitude()); p.put("accuracyM",l.getAccuracy()); p.put("sequenceNo",++sequence); p.put("batteryPct",battery());
    ...
    private Notification notification(String text){ String equipment=prefs().getString("equipmentId","FICO Tracker"); return new NotificationCompat.Builder(this,CHANNEL).setSmallIcon(R.drawable.ic_launcher).setContentTitle(equipment+" · rastreamento ativo").setContentText(text).setOngoing(true).setCategory(NotificationCompat.CATEGORY_SERVICE).build(); }
